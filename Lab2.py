...
def get_user_input():
    ...
    arraylist=[] # define empty list
    numstring=input() 
    numlist=numstring.split(",")
    for eachnum in numlist:
        arraylist.append(float(eachnum)) # convert into float number 
...
def calc_average(arraylist):
    print("calc_average")
    total = sum(arraylist)
    average = total/len(arraylist) #len() function returns the number of items in an object
